...
- 在会话中显式调用 `/knowledge-save`
- 将规则写入全局 `AGENTS.md`，在出现关键进展时自动沉淀到 Obsidian 知识库。这种方式更推荐
...
安装完成并重启客户端后，可以直接在会话中显式调用 `/knowledge-save`。
...
如果你希望它在日常使用中更自然一些，也可以把规则写入全局 `AGENTS.md`，例如要求 AI 在出现关键进展时自动调用 `knowledge-save` 进行沉淀。
